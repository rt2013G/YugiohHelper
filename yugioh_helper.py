...
def main():
    ...
    application.add_handler(CommandHandler("card", handler.card_search_cmd, ~filters.Chat(chat_id=cfg.market_id)))
    ...
    application.add_handler(CommandHandler("resetdate", handler.reset_date))
    application.add_handler(CommandHandler("gdpr", handler.gdpr, filters.ChatType.PRIVATE))
    application.add_handler(CommandHandler("addscammer", handler.add_scammer))
    application.add_handler(CommandHandler("removescammer", handler.remove_scammer))
    application.add_handler(CommandHandler("reject", handler.reject_seller, filters.ChatType.PRIVATE | filters.Chat(chat_id=cfg.approve_group)))
    ...
    application.add_handler(MessageHandler(filters.Chat(chat_id=cfg.market_id) & ~filters.COMMAND & (filters.TEXT | filters.VIDEO | filters.PHOTO), handler.market_msg_updater))
